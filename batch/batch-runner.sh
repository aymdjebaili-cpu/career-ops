...
# Save email drafts to Gmail (runs automatically after batch)
save_gmail_drafts() {
  echo ""
  echo "=== Saving Gmail Drafts ==="
  if node "$PROJECT_DIR/save-drafts.mjs" --score=3.5; then
    echo ""
  else
    local exit_code=$?
    if [[ $exit_code -eq 1 ]]; then
      echo "⚠️  Gmail drafts skipped (not set up or token missing)"
      echo "   Run: node save-drafts.mjs --setup"
    fi
  fi
}
...
main() {
  ...
  # Save Gmail drafts automatically
  save_gmail_drafts
